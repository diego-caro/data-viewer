...
```bash
npx jest --testPathPattern="components|services|pipes" --coverage --coverageReporters=text
```
If <70%: write additional tests to fill gaps, re-run until ≥70% or flag as blocker.
...
For each criterion:
1. Create `/cypress/e2e/SCRUM-[N]-[feature].cy.ts`
2. Cover the happy path
3. Cover at least one error or edge case

Run: `npx cypress test` — all must pass.
