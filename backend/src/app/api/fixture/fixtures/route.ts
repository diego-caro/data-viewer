...
export async function GET(request: NextRequest): Promise<NextResponse<FixtureResponse | { error: string }>> {
  ...
  try {
    const instances = await fixtureService.getFixtures(fixtureId);
    return NextResponse.json({ data: instances });
  } catch {
    return NextResponse.json({ error: 'Failed to fetch fixture results' }, { status: 500 });
  }
}
