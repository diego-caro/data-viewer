import { POST } from '@/app/api/payments/mark-paid/route';
...
function createPostRequest(body: unknown, authHeader?: string): NextRequest {
  ...
  return new NextRequest('http://localhost:3000/api/payments/mark-paid', {
    method: 'POST',
    headers,
...
describe('POST /api/payments/mark-paid', () => {
  ...
  it('should return 404 when player fee not found', async () => {
    mockedUserService.verifyToken.mockReturnValue({ userId: 'admin-1', role: 'admin' });
    mockedPaymentService.markPlayerPaid.mockResolvedValue(null);
